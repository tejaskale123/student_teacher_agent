...
PDF_UPLOADS_DIR = UPLOADS_DIR / "pdfs"
...
def rebuild_rag_for_pdf(file_bytes, original_name):
    ...
    pdf_name = safe_filename(original_name)
    (PDF_UPLOADS_DIR / pdf_name).write_bytes(file_bytes)
...
def handle_uploaded_file(uploaded_file):
    ...
    if file_extension == ".pdf":
        ...
        st.session_state.upload_status = "Saved"
...
def run_teacher(prompt):
    ...
    with st.spinner("Thinking..."):
        try:
            ...
            st.session_state.last_agent_run = st.session_state.teacher.last_run
        except Exception as error:
...
st.markdown(
    ...
    <style>
        ...
        .chatbar-row .stButton > button:hover {
            background: var(--sidebar-hover);
        }

        .chatbar-row [data-testid="stChatInput"] {
            flex: 1;
            margin: 0;
        }

        .chatbar-row [data-testid="stChatInput"] textarea {
            background: transparent;
            border: none;
            min-height: 3rem;
        ...
        .chatbar-row [data-testid="stChatInput"] textarea:focus {
            outline: none;
            box-shadow: none;
        }
...
with st.sidebar:
    ...
    if sidebar_pdf is not None:
        ...
        if file_signature != st.session_state.uploaded_file_signature:
            ...
            try:
                handle_uploaded_file(sidebar_pdf)
                st.session_state.uploaded_preview = sidebar_pdf.name
                st.session_state.upload_status = "Ready"
            except Exception as error:
                st.session_state.upload_status = f"Upload failed: {error}"
            st.rerun()
...
with chat_history:
    ...
    if last_run.get("route") and last_run.get("route") != "N/A":
        col1, col2, col3 = st.columns(3)
        col1.metric("Route", last_run.get("route", "N/A"))
...
if st.session_state.uploaded_preview:
    st.markdown(
        f"<div class='upload-preview'>📄 {st.session_state.uploaded_preview}</div>",
        unsafe_allow_html=True,
    )
